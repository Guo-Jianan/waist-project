...
│   ├── rehab_record.py              # 康复记录页：训练记录管理与 AI 报告回放
│   ├── fun_game.py                  # 预留：游戏交互
...
- **训练记录列表**：按次展示康复训练记录，显示用户名称、训练开始时间
- **报告查看**：每条记录可查看对应 AI 分析报告（HTML 渲染，支持表格、列表、标题等 Markdown 排版）
- **自动关联**：训练结束时自动创建记录，AI 分析结果生成后自动关联到对应记录
- **报告回放**：选中某条记录即可回放历史 AI 分析结果，便于对比多轮训练效果
